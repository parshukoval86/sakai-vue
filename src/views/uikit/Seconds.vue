...
if (store.date[0] == store.date[1]) {
    tit.value = `Показатели секунд на трубу на ${moment(store.date[0]).format('DD.MM.YYYY')}`;
} else {
    tit.value = `Показатели секунд на трубу c ${moment(store.date[0]).format('DD.MM.YYYY')} по ${moment(store.date[1]).format('DD.MM.YYYY')}`;
}

pred.value = 'по каждому ТС';

uprrService.getSeconds(moment(store.date[0]).format('YYYY-MM-DD'), moment(store.date[1]).format('YYYY-MM-DD'),selectedCity.value.name).then((data) => (prrData.value = data.data));
...
const changeDate = () => {
    ...
    if (store.date[0] - store.date[1] == 0) {
        ...
        tit.value = `Показатели секунд на трубу на ${moment(store.date[0]).format('DD.MM.YYYY')}`;
        pred.value = 'по каждому ТС';
        uprrService.getSeconds(moment(d).format('YYYY-MM-DD'), moment(d).format('YYYY-MM-DD'), city).then((data) => (prrData.value = data.data));
    } else {
        start = moment(store.date[0]).format('YYYY-MM-DD');
        end = moment(store.date[1]).format('YYYY-MM-DD');
        if (store.date[1] != null) {
            uprrService.getSeconds(start, end, city).then((data) => (prrData.value = data.data));
            tit.value = `Показатели секунд на трубу c ${moment(store.date[0]).format('DD.MM.YYYY')} по ${moment(store.date[1]).format('DD.MM.YYYY')}`;
    ...
    <div class="grid p-fluid">
        ...
        <div class="col-12 xl:col-12" v-if="loading">
            <div class="card flex flex-column align-items-center">
                <!-- <h5 class="text-left w-full">  Показатели секунд на трубу  {{ tit }}</h5> -->
                <div style="width: 100%">
                    <div id="chart">
